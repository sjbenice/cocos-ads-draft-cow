import { _decorator, animation, CCString, Component, instantiate, Node, Prefab, Quat, random, randomRange, randomRangeInt, SkeletalAnimation, Tween, tween, v3, Vec3 } from 'cc';
...
export class GuestController extends Component {
    ...
    public prepareBuy() {
        ...
        if (this.orderIcon) {
            this.orderIcon.active = true;
            
            tween(this.orderIcon)
            .by(1, {position:v3(0, 0.2, 0)})
            .by(1, {position:v3(0, -0.2, 0)})
            .union()
            .repeatForever()
            .start();
    ...
    public buyGood(node:Node) {
        if (this._orderCount > 0 && node) {
            ...
            if (item) {
                ...
                Utils.calcArrangePos(this._placeHalfDimension, item.getHalfDimension(), this.placePos.children.length - 1, this._tempPos);
    
                ParabolaTween.moveNodeParabola(node, this._tempPos, 2, this.parabolaTime, -1, 0, false);
    ...
    public checkPay(placePos:Node, placeDimen:Vec3) : boolean {
        if (this._state == GuestController.State.PAYING) {
            if (this._totalPrice > 0 && placePos) {
                ...
                ParabolaTween.moveNodeParabola(cash, this._tempPos, 4, this.parabolaTime, -1, 0/*720*/, placePos.children.length > 100, this.arrangeDollarCallback);
    ...
    start() {
        ...
        this.updateAnimation(this.speed);
    ...
    protected updateAnimation(speed:number) {
        let name:string = speed > 0 ? 'walk' : 'idle';
