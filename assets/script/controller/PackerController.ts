...
import { SimpleAssistantController } from './SimpleAssistantController';
...
export class PackerController extends Component {
    ...
    @property(WorkZone)
    activeWorkZone:WorkZone = null;

    @property(SimpleAssistantController)
    packer:SimpleAssistantController = null;
    ...
    update(deltaTime: number) {
        ...
        if (this._timer >= this.packTime) {
            this._timer = 0;

            if (this.packer && this.packer.node.activeInHierarchy)
                this.packer.setWorking(this.inputPos.children.length > 0);

            if (this.activeWorkZone == null || this.activeWorkZone.hasPlayer()) {
