import { _decorator, animation, CCString, Component, EPSILON, instantiate, Node, ParticleSystem, Prefab, Quat, randomRange, randomRangeInt, SkeletalAnimation, SkinnedMeshRenderer, tween, v3, Vec3 } from 'cc';
...
export class FrozenAssistantController extends Component {
                        ...
                        if (this.placePos.children.length == 0) {
                            ...
                            if (!this._moving) {
                                if (this.ghostMgr) {
                                    ...
                                    if (ghost) {
                                        ghost.getComponent(GhostController).receiveFood(this.placePos.children[0]);
                                    } else {
                                        this._moving = true;
                                    }
                                }
    ...
    public setWorkerType(type:number, workerIndex:number, zone:Node) {
        if (this._state == State.FOLLOW && type != WorkerType.None) {
            ...
            this._workInterval = randomRange(0.6, 0.8);
    ...
    protected increaseWorkCount() : boolean {
        this._workCount ++;
        if (this._workCount >= 2) {
            this._workCount = 0;
