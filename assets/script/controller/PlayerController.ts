...
export class PlayerController extends AvatarController {
    ...
    start() {
        ...
        if (this.isBot()) {
            this._state = PlayerController.State.WORK;//TO_OUT
        }
        ...
        if (event.otherCollider && event.otherCollider.getGroup() == PHY_GROUP.OBJECT) {
            if (this.placePos.children.length > 0) {
                ...
                if (otherNode) {
                    ...
                    if (ghost) {
                        ghost.receiveFood(this.placePos.children[this.placePos.children.length - 1]);
                    }
                }
            }
        }
    ...
    protected onTrigger(event: ITriggerEvent, enter:boolean) {
        ...
        if (item && item.enabled) {
            ...
            this.moveBotToOutput();
    }

    public catchItem(item:Item, jumpCount:number = 1, jumpRange:number = 1) : boolean {
                ...
                if (caution || (item.type > 0 && this.itemMax > 0 && placePos.children.length >= this.itemMax)) {
                    ...
                    if (useParabola) {
                        ...
                        if (jumpCount > 1) {
                            ...
                            let delay:number = randomRange(0.2, 0.3);
                            soundDelay += delay;

                            this._tempPos2.x += randomRange(-jumpRange, jumpRange);
                            this._tempPos2.z += randomRange(-jumpRange, jumpRange);
                            const parabola = ParabolaTween.moveNodeParabola(item.node, this._tempPos2, randomRange(0.5, 1.5), delay, -1, randomRange(180, 360), false);

                            for (let index = 0; index < jumpCount - 2; index++) {
                                this._tempPos2.set(item.node.position);
                                this._tempPos2.x += randomRange(-jumpRange, jumpRange);
                                this._tempPos2.z += randomRange(-jumpRange, jumpRange);

                                delay = randomRange(0.2, 0.3);
                                soundDelay += delay;
                                parabola.addPath(this._tempPos2, randomRange(0.5, 1.5), delay, -1, randomRange(180, 360));
                            }

                            delay = 0.5;
                            soundDelay += delay;
                            parabola.addPath(this._tempPos, this.itemParabolaHeight, delay, -1, 360);
                        } else {
                            const delay = 0.5;
                            soundDelay += delay;

                            ParabolaTween.moveNodeParabola(item.node, this._tempPos, this.itemParabolaHeight, delay, -1, 0, false);
                        }
                    } else {
            ...
            switch (this._state) {
                ...
                case PlayerController.State.TO_OUT:
                    if (this.placePos && this.placePos.children.length == 0 && this.inputZone) {
                        this._state = this.workPos ? PlayerController.State.WORK : PlayerController.State.TO_IN;
                        this._moving = true;
                    } else if (this.outputZone)
                        this.calcMoveInput(this.outputZone.getWorldPosition(this._tempPos));
                    break;
                case PlayerController.State.WORK:
                    if (this.workPos) {
                        if (this.workPos.children.length > 0) {
                            this.outputZone.getWorldPosition(this._tempPos);
                            this.node.getWorldPosition(this._tempPos2);
                            if (Vec3.distance(this._tempPos, this._tempPos2) < 0.1) {
                                this._moving = false;
                                Utils.faceViewCommon(Utils.Vec3Backward, deltaTime, this.node, this.turnAngleSpeed);
                            } else {
                                this.calcMoveInput(this._tempPos);
                                this._moving = true;
                            }
                        } else {
                            if (this.inputZone && this.inputZone.children.length > 0) {
...
*/
            ...
            if (animal) {
                if (animal.onFollowing(this.node)) {
                    if (this.placePos) {
                        ...
                        if (good) {
                            if (this.catchItem(good.getComponent(Item), 2)) {
    ...
    protected moveBotToOutput() {
        if (this.isBot() && this.outputZone && this._state == PlayerController.State.TO_IN && this._moving == false) {
