...
export class MyGameMgr extends GameMgr {
    ...
    onLoad(): void {
        ...
        if (this.arrow)
            this._arrowMeshRenderer = this.arrow.getComponentInChildren(MeshRenderer);
    }
    ...
    protected lateUpdate(dt: number): void {
        ...
        if (this.player) {
            ...
            if (item) {
                ...
                if (!done) {
                    for (let index = 0; index < this.moneyGroup.length; index++) {
                        const element = this.moneyGroup[index];
                        if (element && element.activeInHierarchy && element.children.length > 0) {
                            this.setTutorPos(element, false, false, true);
                            this._fieldPosTimer = 0;
                            done = true;
                            break;
                        }
                ...
                if (!done && this.fieldPos) {
                    ...
                    if (/*this._firstFieldTutor ||*/ this._fieldPosTimer > 1.5) {
                        this._firstFieldTutor = false;
                        this._fieldPosTimer = 0;

                        this.setTutorPos(this.fieldPos, false, false, false, 2, 0);
